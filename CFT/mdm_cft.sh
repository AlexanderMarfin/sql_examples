...
#Step 1 Execute TEMP_TABLE.sql
echo "Step 1 Execute TEMP_TABLE.sql"
echo exit | sqlplus -L $1/$DBpass@$TNS @TEMP_TABLE.sql;
if [ "$?" == "1" ] 
then
    echo "_____________________________"
	echo "ERROR! TEMP_TABLE.sql failed!"
    echo "_____________________________"
	exit 1
else
    echo "_____________________________"
	echo "SUCCESS! TEMP_TABLE.sql completed!"
    echo "_____________________________"
fi
#Step 2 Execute LOG_TABLE.sql
echo "Step 2 Execute LOG_TABLE.sql"
echo exit | sqlplus -L $1/$DBpass@$TNS @LOG_TABLE.sql;
if [ "$?" == "1" ] 
then
    echo "_____________________________"
	echo "ERROR! LOG_TABLE.sql failed!"
    echo "_____________________________"
	exit 1
else
    echo "_____________________________"
	echo "SUCCESS! LOG_TABLE.sql completed!"
    echo "_____________________________"
fi
#Step 3 Data transfer
echo "Step 3 Data transfer"
echo exit | sqlldr $1/$DBpass@$TNS control='control.txt' log='loader.txt';
if [ "$?" == "1" ]
then
    echo "_____________________________"
...
#Step 4 Execute MAIN.sql
echo "Step 4 Execute MAIN.sql"
